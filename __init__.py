...
def regularize(DATA_ARRAY,method='fit',fit_dsr=16,gain_order=1,cnst_order=1):
    ...
    if method == 'fit':
        ...
        for i, D in enumerate(DATA_ARRAY):
            #print(D.shape,ref_time.shape,ref_data.shape)
    ...
    return REG_DATA, tot_rms, is_regular, np.array(cal_pars)

def get_sub_splits(self,durations=[]):
        
    xvel_ = np.gradient(self.azim) / np.gradient(self.time) * np.cos(self.elev)
    ...
    splits = np.array([[s,e] for s,e in zip(flags,flags[1:]) if (e-s) > 804]).astype(int)

    # compiles sub-scans that cover the TOD
    self.sub_splits = splits.copy()
    for i,(s,e) in enumerate(splits):

        split_dur = self.time[e] - self.time[s]
