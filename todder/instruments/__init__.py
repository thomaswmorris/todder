...
class TOD():
    ...
    def __init__(self, source, res=None, source_type='moby', downsample_rate=1, downsample_method='triangle', verbose=False):
        ...
        if source_type == 'moby':
            ...
            if source.cuts is None:
                self.cuts = signal.make_cuts(sp.signal.detrend(source.data), downsample_rate=4)
                self.cuts_type = 'auto'
            else: 
            ...
            self.data = signal.apply_cuts(source.data, source.cuts, tol=4, method='splice')
            self.data = signal.downsample(self.data, rate=downsample_rate, method=downsample_method)
            ...
            self.hk = np.c_[source.ctime, signal.unwrap_angle(source.az), source.alt, source.thermometer].T
            self.hk = signal.downsample(self.hk, rate=downsample_rate, method='triangle')
            self.hk = signal.apply_cuts(self.hk, signal.make_cuts(self.hk, downsample_rate=4), method='splice')
    ...
    def post_process(self, mode='minimal'):
        ...
        if mode == 'crop':
            ...
            y = signal.downsample(sp.signal.detrend((self.data / self.data.std(axis=1)[:, None]).mean(axis=0)), rate=16, method='triangle')
        ...
        if mode == 'remove_template':
            ...
            self.phase_template = signal.get_phase_template(self.power, self.phase, n_phase_bins=360, discriminator=self.metadata.pa.values)
