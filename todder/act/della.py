import re, os, glob, os, pickle, pytz
...
def get_scode(t):
    i = np.digitize(t, bins=[1397018846, 1426572705, 1456846608, 1488443939, 1521156461, 1549177098, 1580660694, 1611466488])
    return f's{i+13}'

def get_tod_filepath(tod_id): 
    filepaths = glob.glob(f'{tod_filepath_base}/season*/merlin/{tod_id[:5]}/{tod_id[:-5]}.zip')
...
def get_calibration(tod_id, det_uid, scheme='iv'):
    ...
    if scheme == 'iv':
        ...
        cals  = m2.scripting.get_calibration([{'type': 'iv', 'source': 'data'}], tod_info=tod_info, det_uid=det_uid).get_property('cal')[1]    
    ...
    if scheme == 'bs':
        ...
        import ast
        tod_filepath = glob.glob(f'/projects/ACT/managed/actpol_data/season*/merlin/{tod_id[:5]}/{tod_id[:-5]}.zip')[0]
        ...
        if not len(fps) > 0: return None

        d = {}
        with open(fps[0], 'r') as f: c = f.read()
        for l in c.split('\n'):
            if len(l) > 0:
                k, v = l.split(' = ')
...
def get_cuts(tod_id, det_uid, scheme):
    ...
    pa, nom_freq = re.sub(fr'.*.*\..*\.ar(.):(.*)', r'PA\1 \2', tod_id).split()
    
    if scheme == 'depot': 
        pattern = f'{tod_cuts_base}/{pa.lower()}_{nom_freq}_*/{tod_id[:5]}/{tod_id[:-5]}.cuts'
        fps = glob.glob(pattern)        
        if len(fps) > 0: return m2.tod.cuts.TODCuts.read_from_path(fps[0])
...
def get_flatfield(tod_id, det_uid, scheme):
    ...
    if scheme == 'atm':
        return pd.read_csv(f'/home/tm12/act/products/atmosphere/flatfields/ff_{band}.csv', index_col=0)#.reindex(det_uid)

    if scheme == 'dummy':
        flatfield = pd.DataFrame(index=det_uid); flatfield['ff'] = 1.0 # dummy flatfield
        return flatfield
