...
def unwrap_angle(angle):
    ...
    cntr_angle = .5 * (adju_angle.max() + adju_angle.min())
...
def downsample(DATA, rate, axis=-1, method='triangle'):
    ...
    if method=='flat':
        ...
        if rate < 1: raise ValueError('downsample rate must be an integer >= 1')
            
        _DATA  = np.swapaxes(DATA, 0, axis)
        kernel = np.expand_dims(get_kernel(n=rate, kind=method), axis=tuple(np.arange(1, len(DATA.shape))))
        n_kern = len(kernel)
        starts = np.arange(0, len(_DATA) - n_kern, rate)
        ends   = starts + n_kern

        return np.swapaxes(np.r_[[np.sum(_DATA[s:e]*kernel, axis=0) for s, e in zip(starts, ends)]],0,axis)
    

def get_phase_template(DATA, phase, n_phase_bins, discriminator=None):

    if discriminator is None: discriminator = np.ones(DATA.shape[0])
    ...
    for ud in np.unique(discriminator):
        ...
        template_degree = 2
        poly = PolynomialFeatures(degree=template_degree).fit_transform(np.linspace(-1,1,nt)[:,None])
        ...
        PP = np.concatenate([P * poly[:,i][:,None] for i in range(template_degree+1)], axis=1)
...
def make_cuts(D, n_filt=3, downsample_rate=4, max_cuts=256):
    ...
    for i, _res in enumerate(residual):
        ...
        for sub_index in grouper(np.where(is_bad)[0], tol=2):
            ...
            if s < 0 or e > mnt - 1: continue
            sub_res_sum = _res[sub_index].sum()
            cuts[-1].append((downsample_rate*np.min(sub_index)-1, downsample_rate*np.max(sub_index)+1))
            
        if len(cuts[-1]) > max_cuts: cuts[-1] = [(0, mnt - 1)]
...
def apply_cuts(D, cuts, tol=4, method=None):
    ...
    for i, _cuts in enumerate(cuts):
        for s,e in _cuts :   
            if e - s > 1024:
                fD[i,0] = np.nan; continue
            elif method == 'splice':  
                t0, t1 = np.maximum(s-1,0), np.minimum(e,T-1)
                fD[i,t0:t1] = np.linspace(fD[i,t0],fD[i,t1],t1-t0) 
            elif method == 'flatten':           
                i0,i1,i2,i3 = np.maximum(s-tol,0), s, e, np.minimum(e+tol,T-1)
                if not i0<i1<i2<i3: continue
...
def decompose(DATA, mode='us', downsample_rate=1):
    ...
    if mode == 'us'  : return u, s
    if mode == 'uv'  : return np.matmul(u,np.diag(s)), np.matmul(np.linalg.pinv(np.matmul(u, np.diag(s))), DATA)
    if mode == 'usv' : return u, s, np.matmul(np.linalg.pinv(np.matmul(u, np.diag(s))), DATA)

# bandpassing functions
bandpass = lambda data, lc, hc, fs, order : sp.signal.filtfilt(*sp.signal.butter(order,[2*lc/fs,2*hc/fs],btype='band'),data,axis=-1)
lowpass  = lambda data, c, fs, order : sp.signal.filtfilt(*sp.signal.butter(order,2*c/fs,btype='lowpass'),data,axis=-1)
highpass = lambda data, c, fs, order : sp.signal.filtfilt(*sp.signal.butter(order,2*c/fs,btype='highpass'),data,axis=-1)