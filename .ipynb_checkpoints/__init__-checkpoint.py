import re, os, glob, pickle, pytz, sys
    ...
    if source == 'moby':
        ...
        for _tod_ids in tod_ids:
            ...
            for __tod_id in _tod_ids:
                ...
                if not __tod.data.shape[0] > min_dets: raise NotEnoughDetsError(f'Not enough detectors! (n={__tod.data.shape[0]})')
                    
                # add to our existing TODs
                if _tod is None: _tod = __tod
                else: _tod.append(__tod, dimension='time')
                del __tod, __mtod
                
                if verbose: print(f'loaded {__tod_id} in {ttime.time() - st:.01f}s')
                
            if tod is None: tod = _tod
            else: tod.append(_tod, dimension='dets')
            del _tod
...
class TOD():
    ...
    def __init__(self, source, res=None, source_type='moby', downsample_rate=1, downsample_method='triangle', verbose=False):
                
        if source_type == 'moby':
        
            if source.cuts is None:
                self.cuts = utils.make_cuts(sp.signal.detrend(source.data), downsample_rate=4)
                self.cuts_type = 'auto'
            else: 
            ...
            self.data = utils.apply_cuts(source.data, source.cuts, tol=4, method='splice')
            self.data = utils.downsample(self.data, rate=downsample_rate, method=downsample_method)

            self.metadata = pd.DataFrame(index=np.arange(len(source.det_uid)))            
            for key in source.info.array_data.keys():
                self.metadata.loc[:, key] = np.array(source.info.array_data[key][source.det_uid])
                
            self.metadata.loc[:, 'cals']      = source.calibration.cal.values           
            self.metadata.loc[:, 'cals']     *= 1e12 / source.info.mce_filter.gain()
            self.metadata.loc[:, 'sky_x']     = source.offsets.loc[source.det_uid,'x0'].values
            self.metadata.loc[:, 'sky_y']     = source.offsets.loc[source.det_uid,'y0'].values
            self.metadata.loc[:, 'flatfield'] = source.flatfield.ff.values
            
            self.hk = np.c_[source.ctime, utils.unwrap_angle(source.az), source.alt, source.thermometer].T
            self.hk = utils.downsample(self.hk, rate=downsample_rate, method='triangle')
            self.hk = utils.apply_cuts(self.hk, utils.make_cuts(self.hk, downsample_rate=4), method='splice')
            self.time, self.azim, self.elev, self.thrm = self.hk 
            self.thrm = self.thrm[None]
            
            if np.isnan(self.hk).any(): return

            # cut detectors that are zero or that are calibrated to zero
            self.subset(dets=(self.data[:,:2].std(axis=1) > 0) & (np.abs(self.metadata.cals.values) > 0), verbose=False)

            if not len(self.data) > 0: return

            pa, nom_freq = re.sub(r'.*ar([0-9]):f([0-9])', r'PA\1 \2', source.tod_id).split()
    ...
    def _update_variable_metadata(self):
        ...
        for i, _center in enumerate(centers):
            ...
            if not group_label[i] < 0 : continue

            inds = np.where(np.sqrt(np.square(_center - centers).sum(axis=1)) < max_dist)[0]
    ...
    def _compute_phase(self):
        ...
        f_w = 4
        f, ps = sp.signal.periodogram(self.azim, fs=self.fs, window='hamming'); i_f = ps.argmax()
        self.period = np.sum(ps[i_f-f_w:i_f+f_w]) / np.sum(f[i_f-f_w:i_f+f_w]*ps[i_f-f_w:i_f+f_w])
        self.phase  = np.pi * (sp.signal.sawtooth(2*np.pi*self.time / self.period, width=1) + 1)

        self.phase_mids = np.linspace(0,2*np.pi,360+1)
        self.dphase     = np.gradient(self.phase_mids).mean()
        self.phase_bins = np.append(self.phase_mids[0] - self.dphase/2, self.phase_mids + self.dphase/2)
    ...
    def post_process(self, mode='minimal'):
        ...
        if mode == 'crop':
            ...
            y = utils.downsample(sp.signal.detrend((self.data / self.data.std(axis=1)[:, None]).mean(axis=0)), rate=16, method='triangle')
            x = np.linspace(0, 1, len(y)) 

            p0=[y[0] - y.mean(), 0.01, 0, y.mean()]
            pars, cpars = sp.optimize.curve_fit(exponential, x, y, p0,
        ...
        if mode == 'calibrate':
            ...
            self.power = sp.signal.detrend(self.data * (self.metadata.flatfield.values * self.metadata.cals.values)[:,None])
            self.power = utils.highpass(self.power, c=1e-3, fs=self.fs, order=1)
            
        if mode == 'lax_cut':
            
            keep = self.get_good_dets(field='power', verbose=True, rel_gain_bounds=(1e-2,1e2), rel_residual_bounds=(0,5e-1))
            self.subset(dets=keep, fields=['data', 'power'], verbose=True)
            
        if mode == 'strict_cut':
            
            keep = self.get_good_dets(field='power', verbose=True, rel_gain_bounds=(0.25, 4), rel_residual_bounds=(0.0, 0.25))
            self.subset(dets=keep, fields=['data', 'power'], verbose=True)
            
        if mode == 'remove_template':
            
            self.phase_template = utils.get_phase_template(self.power, self.phase, n_phase_bins=360, discriminator=self.metadata.pa.values)
            self.power -= self.phase_template

    def detrend(self, deg, downsample_rate=16):
        
        pt = np.linspace(0,1,self.nt)
        self.data -= np.poly1d(np.polyfit(pt[::downsample_rate], self.data[:,::downsample_rate].mean(axis=0), deg=deg))(pt)[None,:]
        
    def get_good_dets(self, field, verbose=False, rel_gain_bounds=(0.5, 2), rel_residual_bounds=(0.0, 0.25)):

        downsample_rate = 16 # downsample rate, this speeds things up and doesn't affect accuracy

        DATA = getattr(self, field)

        keep = np.ones(DATA.shape[0]).astype(bool) # mask of detectors to keep, initially all true 
        ...
        for band in np.unique(self.metadata.band): # for all unique passbands ...
            ...
            downsampled_data = utils.downsample(DATA[m], rate=downsample_rate)
            
            data_norm = downsampled_data.std(axis=-1)

            normed_data = downsampled_data / data_norm[:,None] # grab and normalize the data for this passband

            u, s, v = la.svd(normed_data, full_matrices=False) # take the SVD

            first_mode = np.outer(u[:,0],s[0]*v[0]) # the dominant mode from the SVD

            rel_residuals = (normed_data - first_mode).std(axis=1) # RMS of normalized residuals after removing the first mode

            gains = data_norm * u[:,0] * np.sqrt(len(s)) # unitful coupling to normalize first mode
            ...
            g &= (rel_residuals > rel_residual_bounds[0]) & (rel_residuals < rel_residual_bounds[1])
            g &= (rel_residuals < 25 * np.percentile(rel_residuals, q=50)) # cut if it's a lot worse than its band colleagues

            if verbose: print(f'{band} : ({(~g).sum():>3} / {len(g)}) are bad ({1e2*(~g).sum()/len(g):>4.01f}%)')
    ...
    def subset(self, dets=None, samples=None, fields=['data'], verbose=False):
        ...
        if not dets is None:
            ...
            for attr in fields:
                if attr in dir(self): setattr(self, attr, getattr(self, attr)[dets])
            self.metadata = self.metadata.loc[dets]
            
            if verbose: print(f'dets subset : {init_shape} -> {self.data.shape}')
                
            if not self.data.shape[0] > 12: 
                raise NotEnoughDetsError(f'Not enough detectors! (n={self.data.shape[0]} < 12)')
        
        if not samples is None:
            if len(samples) == 2:
                samples = (np.arange(self.nt) > samples[0]) & (np.arange(self.nt) < samples[1])
                
            for attr in [*fields, 'thrm']:
                if attr in dir(self): setattr(self, attr, getattr(self, attr)[:,samples])
            for attr in ['time', 'azim', 'elev', 'phase']:
                setattr(self, attr, getattr(self, attr)[samples])
                
            if verbose: print(f'time subset : {init_shape} -> {self.data.shape}')
    ...
    def append(tod1, tod2, fields=['data'], dimension=None, bins=None):
        ...
        if dimension == 'time':
            ...
            new_det_uid = sorted(list(set(tod1.metadata.det_uid) & set(tod2.metadata.det_uid)))
            ...
            tod1.data = np.c_[tod1.data[tod1_det_index], tod2.data[tod2_det_index] + (tod1.data[tod1_det_index,-16:].mean(axis=-1) - tod2.data[tod2_det_index,:16].mean(axis=-1))[:,None]]
            
            tod1.metadata = tod1.metadata.iloc[tod1_det_index]
            for attr in ['time', 'azim', 'elev', 'phase', 'thrm']:
                if attr in dir(tod1): setattr(tod1, attr, np.concatenate([getattr(tod1,attr), getattr(tod2,attr)], axis=-1))
                    
            # ensures unique time samples
            linear = lambda x, a, b : a * x + b
            pars, cpars = sp.optimize.curve_fit(linear, np.arange(len(tod1.time)), tod1.time)
        ...
        if dimension == 'dets':
            ...
            tod1_t_index = np.round(np.interp(master_time, tod1.time, np.arange(tod1.nt))).astype(int)
            tod2_t_index = np.round(np.interp(master_time, tod2.time, np.arange(tod2.nt))).astype(int)
            ...
            for attr in ['azim', 'elev']:
                ...
                setattr(tod1, attr, np.interp(master_time, tod1.time, getattr(tod1, attr)))
            
            for attr in [*fields, 'thrm']:

                setattr(tod1, attr, np.vstack([getattr(tod1, attr)[:, tod1_t_index], getattr(tod2, attr)[:, tod2_t_index]]))
    ...
    def get_bimodes(self):
        ...
        for pa in np.unique(self.metadata.paba):
            ...
            power_gains        = (pa_power_template * self.power[pam]).sum(axis=1) / np.square(pa_power_template).sum()

            pa_noise_template  = (self.power[pam] - np.outer(power_gains, pa_power_template)).mean(axis=0)
            pa_noise_template /= pa_noise_template.std()
            pa_noise_gains     = (pa_noise_template * self.power[pam]).sum(axis=1) / np.square(pa_noise_template).sum()
    ...
    def cluster(self, n=16, method='kmeans', discriminator=None, field='data', filter_kind='none', flim=[1e-2, 1e1], order=3, min_spacing=np.radians(.1)):
        
        if discriminator is None: discriminator = np.repeat('all', self.nd)
                
        setattr(self, f'c_{field}', np.empty((0, self.nt)))
                
        self.cx, self.cy, self.ci = [], [], []
        
        for attr in ['c_x', 'c_y', 'c_pa', 'c_band']: setattr(self, attr, [])
        for d in np.unique(discriminator):
            ...
            points = np.vstack([self.metadata.sky_x.values[m], self.metadata.sky_y.values[m]]).T
            ...
            if method == 'grid':
                nx, ny = int(np.ptp(self.x[m])/min_spacing), int(np.ptp(self.y[m])/min_spacing)

                x_bins = np.linspace(self.x[m].min()-1e-6,self.x[m].max()+1e-6,nx+1)
                y_bins = np.linspace(self.y[m].min()-1e-6,self.y[m].max()+1e-6,ny+1)
                x_id, y_id = np.digitize(self.x[m],bins=x_bins), np.digitize(self.y[m],bins=y_bins)
                print(nx,ny,x_bins.min(),x_bins.max())
                self.c_id = nx * x_id + y_id

            cx = np.array([self.metadata.sky_x.values[m][self.c_id==i].mean(axis=0) for i in np.sort(np.unique(self.c_id))])
            cy = np.array([self.metadata.sky_y.values[m][self.c_id==i].mean(axis=0) for i in np.sort(np.unique(self.c_id))])
            self.c_n = np.array([np.sum(self.c_id==i) for i in np.sort(np.unique(self.c_id))])
        
            masked_data = getattr(self, field)[m]
            
            setattr(self, f'c_{field}', np.r_[getattr(self, f'c_{field}'), np.concatenate([masked_data[self.c_id==i].mean(axis=0)[None,:] for i in np.sort(np.unique(self.c_id))],axis=0)])
            ...
            for attr in ['pa', 'ba', 'band']: getattr(self, f'c_{attr}').extend(np.repeat(self.metadata[attr].values[m][0], len(cx)))
                       
        for attr in ['c_x', 'c_y', 'c_pa', 'c_ba', 'c_band']: setattr(self, attr, np.array(getattr(self, attr)))
            
        self.n_cluster = len(self.cx)

        if filter_kind == 'band-pass':
            setattr(self, f'c_{field}', _bandpass(getattr(self, f'c_{field}'), *flim, self.fs, order=order))
        if filter_kind == 'low-pass':
            setattr(self, f'c_{field}', _lowpass(getattr(self, f'c_{field}'), flim, self.fs, order=order))
        if filter_kind == 'high-pass':
            setattr(self, f'c_{field}', _highpass(getattr(self, f'c_{field}'), flim, self.fs, order=order))
    ...
    def get_modes(self, dt, fields=['power'], discriminators=[None]):
        ...
        modes['meta']['azim'] = sp.interpolate.interp1d(self.time, self.azim, kind='quadratic')(master_time)
        modes['meta']['elev'] = sp.interpolate.interp1d(self.time, self.elev, kind='quadratic')(master_time)
        ...
        for field, discriminator in zip(fields, discriminators):
            ...
            for d_val in np.unique(discriminator):
                ...
                modes['data'][field][d_val] = sp.interpolate.interp1d(self.time, getattr(self, field)[d_mask].mean(axis=0), kind='quadratic')(master_time)
    ...
    def get_splits(self, min_length=5):
        ...
        pair_lag_splits = [(s,e) for s, e in zip(flags[:-1], flags[1:]) if e - s > min_samples]
        
        return pair_lag_splits
        
    def do_pair_lag(self, splits=None, max_lag=5, field='c_power', method='classic', discriminator=None): 
    
        if splits is None: splits = self.get_splits(min_length=5)
        if discriminator is None: discriminator = np.repeat('all', self.nd)        
        ...
        for split, (s, e) in enumerate(splits):
            ...
            for i, j in zip(idets, jdets):
                ...
                if method == 'classic':
                    ...
                    corr = np.correlate(pl_data[i], pl_data[j], mode='full')[(nt-1)-max_di:(nt-1)+max_di+1]

                rs_corr = sp.interpolate.interp1d(di_sampler, corr, kind='cubic')(us_di_sampler)
        ...
        motion = pd.DataFrame(columns=['t', 'band', 'duration', 'azim_v', 'fit_ang_v_x', 'fit_ang_v_y', 'co_r_2'])
        ...
        for d_val in np.unique(discriminator):
            ...
            for split, (s, e) in enumerate(splits):
                ...
                USE = (self.norm_pair_lag[split] != 0) & d_mask[None,:] & d_mask[:,None]

                if not USE.sum() > 0: continue

                abs_norm_pair_lag_bounds = np.percentile(np.abs(self.norm_pair_lag[split][USE]), q=[5,95])
                ...
                try:
                    ...
                    (fit_avx, fit_avy), cpars = sp.optimize.curve_fit(norm_lag_flat,
                                                                    self.c_da[USE], self.norm_pair_lag[split][USE],
                                                                    p0=[vx0,vy0], bounds=bounds, maxfev=10000)

                    TSS = np.square(self.norm_pair_lag[split][USE]).sum()
                    RSS = np.square(self.norm_pair_lag[split][USE] - norm_lag_flat(self.c_da[USE], fit_avx, fit_avy)).sum()
                    co_r_2 = RSS / TSS

                    if not co_r_2 < 0.5: assert False
                    motion.loc[len(motion)] = self.time[s:e].mean(), (e - s) * self.dt, d_val, azim_v, fit_avx, fit_avy, co_r_2
